...
The work queue. One row per unit of work, ordered by dependency depth: leaves first, so
nothing is ever ported against a target that does not exist yet.

Each row carries the task's **current** status only. The append-only history of how it got
there lives in [`LEDGER.md`](./LEDGER.md).

**This file is updated at the end of every task, in the same session that did the work.**
A task is not finished until its row says so and its `LEDGER.md` entries are written.
...
- Change the status **in place**; the previous value is preserved as a `LEDGER.md` entry,
  not in this row.
- **Never mark a row `DONE` without the four ledger lines** (port / review / fix / test)
  in `LEDGER.md` and a passing test run. A row is `DONE` only when the tests you can run
  are green and every review finding is either applied or rejected in writing.
- Never delete a row. Cancelled work becomes `DONE` with `cancelled: <reason>` in Notes.
- Discovering new work mid-task means adding a row — plus its `TODO` entry in
  `LEDGER.md` — not silently widening the current one.
